...
module.exports = function(config) {
  ...
  function init() {
    ...
    Paymnd.Vendor.getMethodKeys().forEach(function(key) {
      ...
      debug('Init Payment Method: %s', payment.getMethodName());
          ...
          methods.forEach(function(method) {
            switch(method) {
              ...
              case 'get':
                ...
                app.get(helper.getMiddlewarePath(payment), middleware.get.bind(payment));
                break;

              case 'post':
                debug('Single Path POST: %s', helper.getMiddlewarePath(payment));
                app.post(helper.getMiddlewarePath(payment), middleware.post.bind(payment));
                break;
