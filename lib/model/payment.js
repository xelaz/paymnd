...
PaymentSchema.static('getByOrderId', function (orderId, callback) {
  this.findOne({orderId : orderId}, function (err, payment) {
    if (err) {
      ...
      callback(new Paymnd.Error.Payment('Payment with orderId "' + orderId + '" was not found'));
    }
  });
});

PaymentSchema.static('getById', function (id, callback) {
  this.findOne({_id : id}, function (err, payment) {
    if (err) {
      callback(new Paymnd.Error.Model(err));
    } else if(payment) {
      callback(null, payment);
    } else {
      callback(new Paymnd.Error.Payment('Payment with _id "' + id + '" was not found'));
    }
  });
});
...
module.exports = function(connection) {
  // init
  module.exports.Model = connection.model('Payment', PaymentSchema);
};
