...
class PaypalPayment extends PaymentAbstract {
  ...
  refund(payment) {
    ...
    return Paymnd.Model.Transaction.getTransactionByCondition({_payment: {$eq :paymentId}, 'response.state': 'approved'})
      .then((transaction) => {
        return { saleId: transaction.response.transactions[0].related_resources[0].sale.id };
      })
      .then(function(result) {
        ...
        return new Promise(function(res, rej) {
          paypal.sale.refund(result.saleId, {}, {}, function(err, response) {
            if(err) {
              return rej(new Error.PaymentError('Refund Payment call failed', null, {error: err, paymentId: paymentId}));
            }
