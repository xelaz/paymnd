...
PaypalPayment.prototype.middleware = {
  ...
  execute: function(req, res) {
    ...
    this.execute(req.query, {}, function(err, payment) {
      if(err) {
  ...
  cancel: function(req, res) {
    ...
    this.cancel(req.query, {}, function(err, payment) {
      if(err) {
        res.redirect(url.resolve('/', redirectUrl.error) + '?orderId=' + req.query.orderId || 0);
      } else {
 ...
 * @return {Promise}
 */
PaypalPayment.prototype.create = function(payment, option) {
  ...
  // genereate internTxn for identify the payment wit vendor
...
PaypalPayment.prototype.execute = function(data, option, callback) {
  ...
  Paymnd.Model.Transaction.Model.getByInternTxnId(data.iTxn, function(err, transactionPayment) {
    ...
    if(err) {
      return callback(new Paymnd.Error.Model('Payment execute failed', null, {error: err, iTxn: data.iTxn}));
    }
    ...
    paypal.payment.execute(transactionPayment.txn, payer, {}, function (err, resp) {
      ...
      if(err) {
        return callback(new Paymnd.Error.Transaction('Execute Payment API call failed', null, {error: err, paymentId: paymentId}));
      }
  ...
  Paymnd.Model.Transaction.Model.getPaypalSaleId(payment._id, function(err, result) {
    ...
    paypal.sale.refund(result.saleId, {}, {}, function(err, resp) {
      if(err) {
        ...
        return callback(new Paymnd.Error.Transaction('Refund Payment call failed', null, {error: err, paymentId: payment._id}));
      }
      ...
      transaction.save(function(err) {
        if (err) return callback(new Paymnd.Error.Model('Transaction save is failed', null, {error:err, paymentId:payment._id}));
        // alles OKay
        callback(null);
      });
    });
