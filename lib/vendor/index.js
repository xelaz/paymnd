...
Vendor.prototype.create = function(name, config) {
  debug('PaymentName: %s\n  options: %o', name, config);

  var parentOptions = extend({}, this.options),
...
module.exports = function(config) {
  ...
  for(var vendorName in config.vendor) {
    if (config.vendor.hasOwnProperty(vendorName)) {
      vendorConfig = config.vendor[vendorName];

      vendor = new Vendor(vendorName, vendorConfig.options);

      if(!vendorConfig.method) {
        // vendor has single payment
        debug('VendorSingle: %s', vendorName);
        var singlePayment = vendor.create(vendorName, vendorConfig);
        vendor.add(vendorName, singlePayment);
        debug('CreatePaymentMethod: %s', singlePayment.getMethod());
      } else {
        debug('VendorList: %s', Object.keys(vendorConfig.method));
        // vendor has many payments
        for(var paymentName in vendorConfig.method) {
          if (vendorConfig.method.hasOwnProperty(paymentName)) {
            vendor.add(paymentName, vendor.create(paymentName, vendorConfig.method[paymentName]));
          }
        }
      }
      ...
      return Promise.resolve()
        .then(function() {
          return new Promise(function(res, rej) {
            Paymnd.Model.Payment.Model.getByOrderId(data.orderId, function(err, payment) {
              debug('paymentCreate#getPayment: orderId %s', data.orderId);
        ...
        .catch(function(err) {
          debug('Error on get Payment:', err.stack);

          return new Promise(function(res, rej) {
            Paymnd.Model.Payment.Model.create(newPaymentData, function(err, payment) {
              debug('paymentCreate#createPayment: orderId %s', data.orderId);
              err && rej(err) || res(payment);
            });
          });
        })
        ...
        .catch(function(err) {
          ...
          throw new Paymnd.Error.Payment('Could not create Payment: ', null, { error:err, data:data });
        });
    },
